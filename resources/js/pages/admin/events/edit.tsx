...
export default function EventEdit({ event }: Props) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        event_date: event.data.event_date.split('T')[0] + 'T' + event.data.event_date.split('T')[1].substring(0, 5), // Format for datetime-local
        location_name: event.data.location_name,
        google_maps_url: event.data.google_maps_url || '',
        ticket_price: event.data.ticket_price,
        max_capacity: event.data.max_capacity || '',
        images: [] as File[],
        existing_images: event.data.images || [],
        status: event.data.status,
        registration_open: event.data.registration_open,
    });
    ...
    const removeExistingImage = (index: number) => {
        const updatedImages = [...data.existing_images];
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(events.update(event.data).url);
    };
        ...
        <>
            ...
            <AppInputPage>
                ...
                <form onSubmit={handleSubmit} className="flex-1 flex min-w-0">
                    <InputFormLayout
                        title="Edit Event"
                        description={`Currently editing: ${event.data.title}`}
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-4">
                                    ...
                                    >
                                        ...
                                        <span className="text-[10px] font-semibold text-muted-foreground group-hover:text-primary uppercase tracking-tighter">Add More</span>
                                    </button>
                                </div>
                                <input
                                    type="file"
                    ...
                    <div className="p-10 flex flex-col gap-6 max-w-2xl mx-auto">
                        <div className="space-y-4">
                            ...
                            <div className="aspect-video w-full rounded-2xl border-4 border-black overflow-hidden shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] bg-muted">
                                {[...data.existing_images.map(img => `/storage/${img}`), ...previews][0] ? (
                                    <img src={[...data.existing_images.map(img => `/storage/${img}`), ...previews][0]} alt="" className="w-full h-full object-cover" />
                                ) : (
