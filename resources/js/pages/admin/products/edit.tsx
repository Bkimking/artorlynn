...
export default function ProductEdit({ product, categories: initialCategories }: Props) {
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        existing_images: product.data.images || [], // Currently stored images
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(products.update(product.data).url);
    };
    ...
    return (
        <>
            <Head title={`Edit ${product.data.title}`} />
            ...
            <AppInputPage>
                ...
                <form onSubmit={handleSubmit} className="flex-1 flex min-w-0">
                    <InputFormLayout
                        title="Edit Product"
                        description={`Currently editing: ${product.data.title}`}
                ...
                >
                    <div className="p-10 flex flex-col gap-8 max-w-2xl mx-auto">
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {/* Show existing images and new previews */}
                                {[...data.existing_images.map(img => `/storage/${img}`), ...previews].map((p, i) => (
