...
function ReviewModal({ review, onClose, onSave, saving }: { review: Partial<Review> | null; onClose: () => void; onSave: (formData: FormData) => void; saving: boolean }) {
    const [draft, setDraft] = useState<Partial<Review>>(review || { name: "", position: "", content: "", rating: 5, is_featured: false, date: new Date().toISOString().split("T")[0], avatar: null });
...
export default function Reviews() {
    ...
    const openAdd = () => setModal({ name: "", position: "", content: "", rating: 5, is_featured: false, date: new Date().toISOString().split("T")[0], avatar: null });
        ...
        <div className="flex flex-col h-full">
            ...
            {selectedReview ? (
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    <div className="flex items-center gap-4">{selectedReview.avatar ? <img src={selectedReview.avatar} alt={selectedReview.name} className="size-16 rounded-2xl object-cover border-2 border-sidebar-border" /> : <div className={cn("size-16 rounded-2xl flex items-center justify-center text-white font-bold text-xl bg-gradient-to-br", avatarColor(selectedReview.name))}>{getInitials(selectedReview.name)}</div>}<div className="flex-1"><div className="flex items-center gap-2"><h2 className="text-lg font-bold">{selectedReview.name}</h2>{selectedReview.is_featured && <Badge variant="secondary" className="text-[10px] gap-1 px-1.5"><CheckCircle2 className="size-2.5" /> Featured</Badge>}</div>{selectedReview.position && <p className="text-sm text-muted-foreground mt-0.5">{selectedReview.position}</p>}<div className="flex items-center gap-3 mt-1.5"><StarRating value={selectedReview.rating} />{selectedReview.date && <span className="text-xs text-muted-foreground">{formatDate(selectedReview.date)}</span>}</div></div></div>
